...
//----------------------------------------------------------------------------------------
#define FOREACH(it, iterable) for (auto it = cineware::GetForEachIterator<typename std::remove_const<typename std::remove_reference<decltype(iterable)>::type>::type, IS_TRUE_TYPE(cineware::IsForEachIteratorType<decltype(iterable)>(0))>::Get(iterable); it; ++it)
